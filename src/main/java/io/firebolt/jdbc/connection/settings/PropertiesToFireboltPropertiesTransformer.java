...
public class PropertiesToFireboltPropertiesTransformer implements Function<Properties, FireboltProperties> {
    ...
    private static final Set<String> connectionSettingKeys = Arrays.stream(FireboltSessionProperty.values())
            .map(FireboltSessionProperty::getKey).collect(Collectors.toSet());
    private static final Set<String> queryParams = Arrays.stream(FireboltSessionProperty.values())
            .map(FireboltSessionProperty::getKey).collect(Collectors.toSet());
    ...
    public FireboltProperties apply(Properties properties) {
        ...
        String sslRootCertificate = getSetting(properties, FireboltSessionProperty.SSL_ROOT_CERTIFICATE);
        ...
        int validateAfterInactivityMillis = getSetting(properties, FireboltSessionProperty.VALIDATE_AFTER_INACTIVITY_MILLIS);
    ...
    private String getDatabase(Properties properties, boolean usePathAsDb, String path) throws IllegalArgumentException {
        ...
        if (usePathAsDb) {
            if ("/".equals(path)) {
                ...
                if (m.matches()) {
                    ...
                    throw new IllegalArgumentException(String.format("The database path provided is invalid %s", path));
                }
            }
        }
        return database;
    ...
    private <T> T getSetting(Properties info, String key, Object defaultValue, Class<?> clazz) {
        ...
        if (clazz == boolean.class || clazz == Boolean.class) {
            final Boolean boolValue = "1".equals(val) || "0".equals(val) ? "1".equals(val) : Boolean.parseBoolean(val);
