...
public class StatementClientImpl extends FireboltClient implements StatementClient {
	...
	private Map<String, String> getAllParameters(FireboltProperties fireboltProperties,
		...
		if (systemEngine) {
			...
			if (connection.getInfraVersion() >= 2) {
				...
				params.put(FireboltQueryParameterKey.QUERY_LABEL.getKey(), determineQueryLabel(fireboltProperties, statementInfoWrapper)); //QUERY_LABEL
			}
	...
	/**
	 * Each statement will have a query label (most of them will be autogenerated except for the SET query_label explicit statement which will use the specified query label).
	 * When encountering a SET query_label statement we will set it on the firebolt properties and use that one instead the one created on the statementInfoWrapper
	 * @param fireboltProperties
	 * @param statementInfoWrapper
	 * @return
	 */
	private String determineQueryLabel(FireboltProperties fireboltProperties, StatementInfoWrapper statementInfoWrapper) {
		String existingQueryLabel = fireboltProperties.getRuntimeAdditionalProperties().get(FireboltQueryParameterKey.QUERY_LABEL.getKey());
