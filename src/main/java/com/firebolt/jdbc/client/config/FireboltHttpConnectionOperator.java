...
public class FireboltHttpConnectionOperator extends DefaultHttpClientConnectionOperator {
	...
	public FireboltHttpConnectionOperator(@NonNull final Lookup<ConnectionSocketFactory> socketFactoryRegistry,
	...
	public void connect(@NonNull final ManagedHttpClientConnection conn, @NonNull final HttpHost host,
		...
		for (int i = 0; i < addresses.length; i++) {
			...
			this.setSocketOption(sock, ExtendedSocketOptions.TCP_KEEPIDLE, fireboltProperties.getTcpKeepIdle());
			this.setSocketOption(sock, ExtendedSocketOptions.TCP_KEEPCOUNT, fireboltProperties.getTcpKeepCount());
			this.setSocketOption(sock, ExtendedSocketOptions.TCP_KEEPINTERVAL, fireboltProperties.getTcpKeepInterval());
			...
			log.debug("{} connect to {} timed out. Connection will be retried using another IP address",
					ConnPoolSupport.getId(conn), remoteAddress);
		}
	}
