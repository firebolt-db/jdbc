...
public enum FireboltSessionProperty {
	...
	BUFFER_SIZE("buffer_size", 65536, Integer.class, false, "The buffer used to create the ResultSet in bytes"),
	SSL("ssl", true, Boolean.class, false, "Enable SSL/TLS for the connection"),
	SSL_CERTIFICATE_PATH("ssl_certificate_path", "", String.class, false, "SSL/TLS root certificate", "sslrootcert"),
	SSL_MODE("ssl_mode", "strict", String.class, false,
			"SSL mode to verify/not verify the certificate. Supported Types: none (don't verify), strict (verify)",
			"sslmode"),

	CLIENT_BUFFER_SIZE("client_buffer_size", 65536, Integer.class, false,
			...
			"apache_buffer_size"),
	MAX_RETRIES("max_retries", 3, Integer.class, false,
	...
	TIME_TO_LIVE_MILLIS("time_to_live_millis", 60 * 1000, Integer.class, false,
			...
			"timeToLiveMillis"),
	MAX_CONNECTIONS_PER_ROUTE("max_connections_per_route", 500, Integer.class, false,
			"Maximum total connections per route", "defaultMaxPerRoute"),
	MAX_CONNECTIONS_TOTAL("max_connections_total", 10000, Integer.class, false, "Maximum total connections",
			"maxTotal"),
	...
	TCP_KEEP_IDLE("tcp_keep_idle", 60, Integer.class, false,
	...
	TCP_KEEP_COUNT("tcp_keep_count", 10, Integer.class, false,
	...
	LOG_RESULT_SET("log_result_set", false, Boolean.class, false,
	...
	FireboltSessionProperty(String key, Object defaultValue, Class<?> clazz, boolean deprecated, String description,
			String... aliases) {
		this.key = key;
	...
	public static List<FireboltSessionProperty> getNonDeprecatedProperties() {
		return Arrays.stream(FireboltSessionProperty.values()).filter(property -> !property.deprecated)
